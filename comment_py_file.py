...
API_langage = {"en": "You are able to write doc-strings respecting PEP 7 and PEP 287 by adding them to the "
               ...
               "fr": "Vous êtes capable d’écrire des doc-string en respectant la PEP 7 et PEP 287 en les "
...
def add_python_docstring(code_str):
    ...
    functions_names = noms_fonctions_dans_code(code_str)
    nb_func = len(functions)
    for function_str, start_index in functions:
        print("Function untraited : " + str(nb_func))
        function_name = noms_fonctions_dans_code(function_str)
        ...
        doc_string = GPT_choice("Turbo", "docstring python", function_str)
        short_docstring = GPT_choice("Turbo", "short docstring", doc_string)
...
def noms_fonctions_dans_code(code_str):
    ...
    indent = len(get_indentation(code_str))
    if indent > 0:
        code_str = indent_code_str(code_str, -indent)
...
def comment_python_functions(code_str):
    """
...
def watch_push_code_here():
    ...
    push_code_here_path = "./Push_code_here"
    if not os.path.exists(push_code_here_path):
        print("Le dossier \"Push_code_here\" n'existe pas")
        os.mkdir(push_code_here_path)
        print("Dossier Push_code_here créée")
        return
    ...
    for item in os.listdir(push_code_here_path):
        ...
        if os.path.isfile(item_path):
            ...
            shutil.copy(item_path, original_path)
            ...
            if file_extension == ".py":
                ...
                modified_code, short_resume = add_python_docstring(code)
                modified_code = autopep8.fix_code(modified_code)
                # Write the new code to the file
                with open(os.path.join(modified_path, item), "w") as file:
                    file.write(modified_code)
                # commente le code complet :
                comment_full_code(os.path.join(
                    modified_path, item), short_resume)
                # Vérifie et commente d'eventuelle lignes non commentées
                correct_py_file(os.path.join(modified_path, item))
            # C part
            elif file_extension == ".c":
                # Read the file
                with open(item_path, "r") as file:
                    code = file.read()
                modified_code = comment_c_function(code)
                # Write the new code to the file
                with open(os.path.join(modified_path, item), "w") as file:
                    file.write(modified_code)
                ...
                if python_convertion == "y":
                    ...
                    file = file+".py"
                    with open(os.path.join(modified_path, file), "w") as file:
                        file.write(python_code)
                    print("File "+item+" converted in python -> " + str(file))
            # Supprimez le fichier de "Push code here"
            os.remove(item_path)
            print(item + " finish")
        # Si l'élément est un dossier, parcourez-le récursivement
        elif os.path.isdir(item_path):
            watch_push_code_here(item_path)
...
def format_langage(langage):
    ...
    if langage.lower() == "docstring python":
        ...
        formated = {"langue": "Python 3.7", "com1": "#", "com2": "\"\"\"", "start": "def ",
                    "prompt": "# Convert the above function respecting PEP 7 and PEP 287 convention:\n",
                    "role": "You must summary in 10 words the python docstring below", "stop": ["def"], "engine": "text-davinci-003"}

    elif langage.lower() == "add python":
        formated = {"langue": "Python 3.7", "com1": "#", "com2": "\"\"\"", "start": "def ",
                    "prompt": "# Convert the above function respecting PEP 7 and PEP 287 convention:\n",
                    "role": API_langage['en'], "stop": ["def"], "engine": "text-davinci-003"}
        ...
        formated = {"langue": "Python 3.7", "com1": "'''", "com2": "'''", "start": "\"\"\"",
                    "prompt": "# An elaborate, high quality docstring for the above c function:\n",
                    "role": "You are an expert in python programming, you must determine the usefulness of this program. your result must be a python comment", "stop": ["\"\"\"", "#"], "engine": "code-davinci-002"}
    else:
        raise ValueError("Langage non pris en charge")
    return formated


def GPT_choice(engine: str = "Turbo", langage: str = "Python", function_or_method: str = ""):
    """
    ...
    function = None
    if engine == "Turbo":
        function = GPT_turbo(langage, function_or_method)
    elif engine == "text-davinci-003":
        function = GPT_classic(langage, function_or_method)
    elif engine == "code-davinci-002":
        function = GPT_classic(langage, function_or_method)
    return function


def GPT_classic(langage, function_or_method):
    """
...
def GPT_turbo(langage, function_or_method):
    """
...
def comment_c_function(code_str):
    """
    ...
    """
    new_code_str = code_str
    functions = extract_C_functions(code_str)
    nb_func = len(functions)
    for function in functions:
        print("Function untraited : " + str(nb_func))
        # opt_function = optimize_token_c(function)
        opt_function = function
        docstring = GPT_choice("code-davinci-002", "C", opt_function)
        # Diviser la chaîne en plusieurs lignes
...
def comment_error_line(file_name):
    """
    ...
    """
    error_line = compile_py_file(file_name)
    if error_line:
...
def correct_py_file(file_name):
    ...
    '''
    code = open_py_file(file_name)
    while True:
        line_error = compile_py_code(code)
...
def extract_return_var_names(child_node):
    '''
    ...
    if isinstance(child_node, ast.Return):
        ...
        if isinstance(child_node.value, ast.Tuple):
            for elt in child_node.value.elts:
                var_names += extract_return_var_names(elt)
        ...
        for sub_node in ast.iter_child_nodes(child_node):
            var_names += extract_return_var_names(sub_node)
    return var_names


def comment_full_code(file_path, short_resume):
    ...
    for node in ast.walk(parsed_tree):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            ...
            for child_node in ast.iter_child_nodes(node):
                var_names += extract_return_var_names(child_node)
            ...
            functions_and_methods.append(info_str)
    dependence = get_dependencies(file_contents)
    resume = '\n'.join(functions_and_methods)
    resume_code = ""
    for dep in dependence:
        resume_code += "import " + dep + "\n"
    resume_code += resume + "\n" + "Summary of function : \n" + short_resume
    reponse = GPT_choice("Turbo", "Python full code", resume_code)
